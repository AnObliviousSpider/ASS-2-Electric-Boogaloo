...
# play(), crossfade_to(), switch_track_at_current_position(),
# stop(), and stop_audio().
...
# Plays audio with options for single playback,
# fading, start position, crossfade, looping, and pitch.
#
# pitch_scale:
# 1.0 = normal pitch
# Below 1.0 = lower pitch
# Above 1.0 = higher pitch
...
func play(
	...
	var safe_start_position: float = maxf(
		start_position,
		0.0
	)

	var safe_pitch_scale: float = maxf(
		pitch_scale,
		0.01
	)
	...
	# Do not restart the same single track
	# from the beginning.
	if (
		single
		and is_playing(audio)
		and safe_start_position <= 0.0
	):
		return

	# Single and crossfade calls cancel older
	# delayed single or crossfade calls.
	if single or crossfade:
		play_token += 1
		local_play_token = play_token

	if crossfade:
		_crossfade_to_player(
			audio,
			fade_duration,
			volume_db,
			safe_start_position,
...
# Switches to another track while keeping
...
func get_current_playback_position(
	use_hardware_clock: bool = true
) -> float:
	var stream_player: AudioStreamPlayer = (
		_get_active_playing_player()
	)
	...
	var current_position: float = (
		stream_player.get_playback_position()
	)

	# Hardware clock gives better timing
	# for synced music switching.
...
# Checks whether a specific audio stream
...
func stop(
	fade_out: bool = false,
	fade_duration: float = 0.5,
	cancel_pending_single_plays: bool = true
) -> void:
...
# Handles smooth crossfading between
# old players and a new player.
...
func _crossfade_to_player(
	...
	var old_players: Array[AudioStreamPlayer] = (
		_get_players(true)
	)
	...
	last_synced_playback_position = maxf(
		start_position,
		0.0
	)
	...
	tween.tween_method(
		_update_equal_power_crossfade.bind(
			new_player,
			old_players,
			old_start_volumes,
			volume_db
...
func _update_equal_power_crossfade(
	...
	var safe_blend: float = clampf(
		blend,
		0.0,
		1.0
	)

	# Equal-power fade avoids a quiet dip
	# in the middle.
	var old_gain: float = cos(
		safe_blend * PI * 0.5
	)

	var new_gain: float = sin(
		safe_blend * PI * 0.5
	)

	_set_player_volume_with_gain(
		new_player,
		target_volume_db,
		new_gain
	)
	...
	for old_player: AudioStreamPlayer in old_players:
		if (
			old_player == null
			or old_player == new_player
		):
			continue

		var old_volume_db: float = (
			old_start_volumes.get(
				old_player,
				old_player.volume_db
			)
		)
...
func _set_player_volume_with_gain(
	...
	var final_linear_volume: float = (
		db_to_linear(base_volume_db)
		* gain
	)
	...
	stream_player.volume_db = linear_to_db(
		final_linear_volume
	)
...
func _start_player(
	...
	tween.tween_callback(
		_erase_fade_tween.bind(
			stream_player
		)
	)


# Sets up a player with audio, volume,
# loop behaviour, pitch, and original-audio tracking.
...
func _prepare_player(
	...
	stream_player.pitch_scale = maxf(
		pitch_scale,
		0.01
	)

	stream_player.stream = _make_playback_audio(
		audio,
		loop
	)

	stream_player.set_meta(
		SOURCE_AUDIO_META_KEY,
		audio
	)
...
# Gets the active player used for
# synced playback position.

func _get_active_playing_player() -> AudioStreamPlayer:
	if (
		last_synced_stream_player != null
		and last_synced_stream_player.playing
	):
...
func _kill_fade_tween(
	...
	var old_tween: Tween = (
		fade_tweens.get(stream_player) as Tween
	)
	...
	# Crossfades can store the same tween
	# on multiple players.
...
func _clear_player(
	...
	if stream_player.has_meta(
		SOURCE_AUDIO_META_KEY
	):
		stream_player.remove_meta(
			SOURCE_AUDIO_META_KEY
		)
...
# Duplicates audio and applies
# the wanted loop setting.

func _make_playback_audio(
	audio: AudioStream,
	loop: bool
) -> AudioStream:
	var playback_audio: AudioStream = (
		audio.duplicate() as AudioStream
	)

	if playback_audio == null:
		playback_audio = audio

	if _audio_has_property(
		playback_audio,
		"loop"
	):
		playback_audio.set(
...
# Checks whether an AudioStream
# has a specific property.

func _audio_has_property(
	audio: AudioStream,
...
# Checks if a player is using
# a specific original audio stream.

func _player_matches_audio(
	stream_player: AudioStreamPlayer,
	audio: AudioStream
) -> bool:
	...
	return (
		stream_player.has_meta(
			SOURCE_AUDIO_META_KEY
		)
		and stream_player.get_meta(
			SOURCE_AUDIO_META_KEY
		) == audio
	)
