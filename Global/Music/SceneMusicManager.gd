...
# Values can be AudioStreams, resource paths, or null.
# Null entries can safely be assigned later in the Inspector.
@export var scene_music: Dictionary = {
	"main_menu": preload(
		"res://Assets/Audio/OST/Final/Main_Menu_Music.wav"
	),

	# Kept so existing calls using "main" still work.
	"main": preload(
		"res://Assets/Audio/OST/Final/Level_1_Normal.wav"
	),

	"main_start": preload(
		"res://Assets/Audio/OST/Level_1_Normal.wav"
	),

	"main_start_glitched": "res://Assets/Audio/OST/Level_1_.Glitched.wav",

	"level_3_normal": "res://Assets/Audio/OST/Level_2_Normal.wav",

	"level_3_glitched": "res://Assets/Audio/OST/Level_2_Glitched.wav",

	"level_4": null,

	"credits": preload(
		"res://Assets/Audio/OST/Final/Credits_Nanners.wav"
	),
}
...
func prepare_scene_music(
	scene_key: String,
	scene_path: String = ""
) -> void:
	prepared_music = null

	if _scene_is_in_list(
		scenes_that_stop_music,
		scene_key,
		scene_path
	):
		stop_music()
	...
	if _scene_is_in_list(
		scenes_that_keep_current_music,
		scene_key,
		scene_path
	):
		return

	var target_music: AudioStream = get_music_for_scene(
		scene_key,
		scene_path
	)
...
func get_music_for_scene(
	...
	if (
		not scene_key.is_empty()
		and scene_music.has(scene_key)
	):
		music_value = scene_music[scene_key]

	elif (
		not scene_path.is_empty()
		and scene_music.has(scene_path)
	):
	...
	# Allows keys to exist before their music
	# has been assigned.
	if music_value == null:
		return null
	...
	if music_value is String:
		var music_path: String = music_value
		...
		push_error(
			"Scene music path is not an AudioStream: %s"
			% music_path
		)
		return null
	...
	push_error(
		(
			"Scene music value for '%s' must be "
			+ "an AudioStream, resource path, or null."
		)
		% scene_key
	)
...
# Checks whether a scene key or path is in a rule list.
func _scene_is_in_list(
	...
	if (
		not scene_key.is_empty()
		and scene_key in scene_list
	):
		return true

	if (
		not scene_path.is_empty()
		and scene_path in scene_list
	):
		return true
